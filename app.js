...
function renderMenu() {
  ...
  $('#menuGrid').innerHTML = products.filter(p => activeCategory === 'workshop' ? ['workshop','workshop-card'].includes(p.category) : p.category === activeCategory).map(p => { const left=p.charityType?charitySlotsLeft(p.charityType):null, soldOut=left===0, detail=p.charityType?`${p.detail}・剩 ${left} 位`:p.detail, image=p.photo ? `<span class="product-photo"><img src="assets/menu/${p.photo}" alt="${p.name} 飲品照片" style="object-position:${p.focus}" /></span>` : `<span class="emoji">${p.emoji}</span>`, product=`<button class="product" data-id="${p.id}" ${soldOut?'disabled':''}>${image}<span class="product-info"><strong>${p.name}</strong><small>${detail}</small><b>${soldOut?'已額滿':money(p.price)}</b></span><span class="add">+</span></button>`; return activeCategory==='drink'?`<div class="drink-product">${product}<button class="store-drink" data-store-product="${p.id}">＋ 寄杯</button></div>`:product; }).join('');
}
...
function addStoredDrinkProduct(id) { const product=products.find(item=>item.id===id); if(!product || product.category!=='drink')return; const storedId=`stored-${id}`, line=cart.find(item=>item.id===storedId); line ? line.qty++ : cart.push({...product,id:storedId,name:`${product.name} 寄杯`,category:'stored-drink',storedProductId:product.id,qty:1}); renderCart(); }
...
function createMemberships(order) {
  ...
  order.items.filter(i => i.category === 'card' || i.category === 'workshop-card' || i.category === 'stored-drink').forEach(item => {
    if (item.category==='stored-drink') {
      memberships.push({id:`membership-${Date.now()}-${Math.random().toString(16).slice(2)}`,customer:order.customer,name:item.name,type:'寄杯',total:item.qty,remaining:item.qty,tea:'',bookingCode:'',usedGrids:[],storedProductId:item.storedProductId||'',purchasedAt:order.date,expiresAt:'無使用期限'});
      return;
    }
