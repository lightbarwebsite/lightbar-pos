const products = [
  ...
  { id:'workshop-five', category:'workshop-card', emoji:'📅', name:'好好生活工作坊五格套票', detail:'可使用 5 次・每堂含 1 杯 NT$150 免費飲品', price:2800 },
...
let activeCategory = 'drink'; let cart = []; let vipActive = false; let vipGrid = null; let vipSelections = { flavor:[], boost:[] }; let selectedWorkshopEvent = null; let managedCalendarEventId = null;
...
const isWorkshopPast = event => new Date(`${event.date}T${event.end || event.start}:00+08:00`).getTime() <= Date.now();
...
function normalizeWorkshopCardNames() {
  const memberships=getMemberships(); let changed=false;
  memberships.forEach(member=>{if(member.type==='工作坊'&&Number(member.total)===5&&/五次套票|五格套票/.test(member.name)&&member.name!=='好好生活工作坊五格套票'){member.name='好好生活工作坊五格套票';changed=true;}});
  if(changed) saveMemberships(memberships);
}
...
function renderMenu() {
  ...
  if (activeCategory === 'workshop') {
    ...
    $('#menuGrid').innerHTML = getWorkshopEvents().sort((a,b)=>`${a.date}${a.start}`.localeCompare(`${b.date}${b.start}`)).map(event=>{const used=bookings.filter(b=>b.eventId===event.id).length, left=event.capacity-used, selected=selectedWorkshopEvent?.id===event.id,past=isWorkshopPast(event); return `<button class="workshop-event ${selected?'selected':''} ${past?'past':''}" data-workshop-event="${event.id}" ${left<1||past?'disabled':''}><span>${event.date.replace('2026-','')}（${['日','一','二','三','四','五','六'][new Date(`${event.date}T12:00`).getDay()]}）</span><strong>${event.title}</strong><small>${event.start}–${event.end}</small><b>${past?'已結束':left?`剩 ${left} 位`:'已額滿'}</b></button>`;}).join('');
    return;
  }
  if (activeCategory === 'calendar') {
    ...
    const dates=Array.from({length:30},(_,i)=>{const day=i+1,date=`2026-09-${String(day).padStart(2,'0')}`, events=getWorkshopEvents().filter(event=>event.date===date).sort((a,b)=>a.start.localeCompare(b.start)); return `<section class="calendar-day"><strong class="calendar-date">${day}</strong>${events.map(event=>{const guests=bookings.filter(b=>b.eventId===event.id),left=event.capacity-guests.length,selected=selectedWorkshopEvent?.id===event.id,past=isWorkshopPast(event);return `<button class="calendar-event ${selected?'selected':''} ${past?'past':''}" data-workshop-event="${event.id}" ${left<1&&!past?'disabled':''}><b>${event.start} ${event.title}</b><small>${past?'已結束（可查看出席）':`剩 ${left} 位`}</small><p class="guest-list ${guests.length?'booked':'empty'}">${guests.length?guests.map(g=>g.customer).join('、'):'尚無預約'}</p></button>`}).join('')}</section>`;}).join('');
...
function syncVipMemberships() {
  ...
  const coachInfo=$('#vipCoachInfo');
  if (selected) {
    $('#vipCardType').value = String(selected.total); vipGrid = selected.usedGrids.length + 1;
    coachInfo.textContent=`所屬教練：${selected.coach||'未指定'}`;
...
function drinkLineModeMarkup(item) {
  if(item.category!=='drink'&&item.category!=='stored-drink') return '';
  const value=item.category==='drink'?'regular':item.storedProductId?'product':'price';
  return `<label class="cart-drink-mode">本杯用途<select data-cart-drink-mode="${item.id}"><option value="regular" ${value==='regular'?'selected':''}>現場飲用</option><option value="product" ${value==='product'?'selected':''}>指定寄杯</option><option value="price" ${value==='price'?'selected':''}>同價任選寄杯</option></select></label>`;
}
function setCartDrinkMode(lineId, mode) {
  const line=cart.find(item=>item.id===lineId); if(!line)return;
  const sourceId=line.sourceProductId||line.storedProductId||line.id.replace(/^stored-(?:product|price)-/,''), product=products.find(item=>item.id===sourceId)||products.find(item=>item.category==='drink'&&item.price===line.price);
  if(!product)return;
  const qty=line.qty;
  if(mode==='regular') Object.assign(line,{...product,id:product.id,name:product.name,category:'drink',qty});
  else { const isPrice=mode==='price'; Object.assign(line,{...product,id:`stored-${isPrice?'price':'product'}-${product.id}`,name:isPrice?`同價任選寄杯 NT$${product.price}`:`${product.name} 寄杯`,category:'stored-drink',storedProductId:isPrice?'':product.id,storedPrice:isPrice?product.price:0,sourceProductId:product.id,qty}); }
  renderCart();
}
function renderCart() {
...
function resetOrder() { cart=[]; vipActive=false; activeCategory='drink'; selectedWorkshopEvent=null; vipGrid=null; vipSelections={flavor:[],boost:[]}; $('#customerName').value=''; $('#saleCoach').value=''; $('#vipMemberPicker').innerHTML='<option value="">請選擇會員</option>'; $('#vipMembership').innerHTML='<option value="">手動登記卡別</option>'; $('#vipCoachInfo').classList.add('hidden'); $('#workshopMemberPicker').innerHTML='<option value="">請選擇會員</option>'; $('#workshopMembership').innerHTML='<option value="">請先選擇會員</option>'; $('#drinkCreditCustomer').innerHTML='<option value="">不使用寄杯</option>'; $('#drinkCreditMembership').innerHTML='<option value="">請先選擇客人</option>'; $('#bookingDate').value=''; $('#bookingContact').value=''; $('#orderNote').value=''; document.querySelectorAll('[data-vip-choice] button').forEach(b=>b.classList.remove('active')); document.querySelectorAll('.tab').forEach(t=>t.classList.toggle('active',t.dataset.category==='drink')); renderVipControls(); renderMenu(); renderCart(); }
function todayOrders() { return getOrders().filter(o=>o.date===today()); }
function monthSalesSummary(month) {
  const orders=getOrders().filter(order=>String(order.date||'').startsWith(month));
  const byDay={}; const products={};
  orders.forEach(order=>{
    const day=order.date;
    if(!byDay[day])byDay[day]={revenue:0,drinks:0,vip:0,orders:0};
    const row=byDay[day]; row.revenue+=Number(order.total||0); row.orders+=1;
    if(order.vip){row.vip+=1;products['VIP 兌換套餐']=(products['VIP 兌換套餐']||0)+1;}
    (order.items||[]).forEach(item=>{if(item.category==='drink')row.drinks+=Number(item.qty||0);if(item.category!=='vip')products[item.name]=(products[item.name]||0)+Number(item.qty||0);});
  });
  const totals=Object.values(byDay).reduce((sum,row)=>({revenue:sum.revenue+row.revenue,drinks:sum.drinks+row.drinks,vip:sum.vip+row.vip,orders:sum.orders+row.orders}),{revenue:0,drinks:0,vip:0,orders:0});
  return {orders,byDay,products,totals};
}
function showMonthlyReport() {
  const picker=$('#monthlyReportMonth'); if(!picker.value)picker.value=today().slice(0,7);
  const month=picker.value, report=monthSalesSummary(month), label=month.replace('-',' 年 ')+' 月';
  const dailyRows=Object.entries(report.byDay).sort((a,b)=>b[0].localeCompare(a[0]));
  const productRows=Object.entries(report.products).sort((a,b)=>b[1]-a[1]);
  const orderRows=report.orders.slice().sort((a,b)=>String(b.createdAt||`${b.date}T${b.time||''}`).localeCompare(String(a.createdAt||`${a.date}T${a.time||''}`)));
  const orderGroups=orderRows.reduce((groups,order)=>{(groups[order.date]??=[]).push(order);return groups;},{});
  const orderDetail=order=>{
    const lines=order.vip?[{name:`VIP 兌換套餐・${order.vip.cardType} 格卡第 ${order.vip.grid} 格`,amount:0}]:((order.items||[]).filter(item=>item.category!=='vip').map(item=>({name:`${item.name} × ${item.qty}`,amount:Number(item.price||0)*Number(item.qty||0)})));
    return `<article class="monthly-order-entry"><div class="monthly-order-head"><div><strong>${order.date}</strong><small>${order.time||''}｜${order.customer||'未填姓名'}</small></div><b>${money(Number(order.total||0))}</b></div><div class="monthly-order-items">${lines.length?lines.map(line=>`<div><span>${line.name}</span><strong>${line.amount?money(line.amount):'兌換'}</strong></div>`).join(''):'<div><span>未填品項</span><strong>${money(Number(order.total||0))}</strong></div>'}</div><button class="danger" data-delete-order="${order.id}" data-delete-context="monthly">刪除這筆</button></article>`;
  };
  $('#monthlyReportContent').innerHTML=`<p class="monthly-report-note">${label}的所有已同步銷售與使用紀錄。</p><div class="report-grid"><div class="metric"><span>整月營收</span><strong>${money(report.totals.revenue)}</strong></div><div class="metric"><span>飲品杯數</span><strong>${report.totals.drinks} 杯</strong></div><div class="metric"><span>VIP 兌換套餐</span><strong>${report.totals.vip} 份</strong></div><div class="metric"><span>訂單筆數</span><strong>${report.totals.orders} 筆</strong></div></div><h3>每日銷售狀況</h3><div class="report-list">${dailyRows.length?dailyRows.map(([date,row])=>`<div class="report-row monthly-day-row"><span><strong>${date}</strong><small>飲品 ${row.drinks} 杯・VIP 套餐 ${row.vip} 份・${row.orders} 筆訂單</small></span><strong>${money(row.revenue)}</strong></div>`).join(''):'<p class="empty-cart">這個月份尚無已同步的訂單紀錄</p>'}</div><h3>品項／套餐統計</h3><div class="report-list">${productRows.length?productRows.map(([name,qty])=>`<div class="report-row"><span>${name}</span><strong>${qty} 份</strong></div>`).join(''):'<p class="empty-cart">這個月份尚無品項紀錄</p>'}</div><h3>逐筆訂單 <small>點日期展開詳細紀錄</small></h3><div class="monthly-order-list">${orderRows.length?Object.entries(orderGroups).map(([date,orders])=>{const dailyTotal=orders.reduce((sum,order)=>sum+Number(order.total||0),0);return `<details class="monthly-order-day" ${date===today()?'open':''}><summary><span><strong>${date}</strong><small>${orders.length} 筆訂單</small></span><b>${money(dailyTotal)}</b></summary><div class="monthly-order-day-content">${orders.map(orderDetail).join('')}</div></details>`;}).join(''):'<p class="empty-cart">這個月份尚無可刪除訂單</p>'}</div>`;
  if(!$('#monthlyReportDialog').open)$('#monthlyReportDialog').showModal();
}
function createMemberships(order) {
  ...
  order.items.filter(i => i.category === 'card' || i.category === 'workshop-card' || i.category === 'stored-drink').forEach(item => {
    if (item.id==='special-mix-card') {
      for(let n=0;n<item.qty;n++) ['250','250','180','150','150'].forEach((price,index)=>memberships.push({id:`membership-${Date.now()}-${n}-${index}-${Math.random().toString(16).slice(2)}`,customer:order.customer,coach:order.coach||'',name:`特調卡同價任選寄杯（NT$${price}）`,type:'寄杯',total:1,remaining:1,tea:'',bookingCode:'',usedGrids:[],storedPrice:Number(price),purchasedAt:order.date,expiresAt:'無使用期限'}));
      return;
    }
    if (item.category==='stored-drink') {
      memberships.push({id:`membership-${Date.now()}-${Math.random().toString(16).slice(2)}`,customer:order.customer,coach:order.coach||'',name:item.name,type:'寄杯',total:item.qty,remaining:item.qty,tea:'',bookingCode:'',usedGrids:[],storedProductId:item.storedProductId||'',storedPrice:item.storedPrice||0,purchasedAt:order.date,expiresAt:'無使用期限'});
      return;
    }
    const total = item.units || (item.category === 'workshop-card' ? 5 : 5);
    const type = item.category === 'workshop-card' ? '工作坊' : 'VIP';
    for (let n=0; n<item.qty; n++) { const purchasedAt=order.date, expiresAt=membershipExpiryDate({type,purchasedAt}); memberships.push({ id:`membership-${Date.now()}-${Math.random().toString(16).slice(2)}`, customer:order.customer, coach:order.coach||'', name:item.name, type, total, remaining:total, tea:item.tea||'', charityType:item.charityType||'', bookingCode:type==='工作坊'?makeBookingCode():'', usedGrids:[], purchasedAt, expiresAt }); }
  });
...
function eligibleWorkshopDrinks() { return products.filter(item=>item.category==='drink'&&item.price===150); }
function renderCalendarManager(event) { const bookings=getWorkshopBookings().filter(booking=>booking.eventId===event.id), available=getMemberships().filter(member=>member.type==='工作坊'&&member.remaining>0&&!isMembershipExpired(member)&&!bookings.some(booking=>booking.membershipId===member.id)), full=bookings.length>=event.capacity, drinkOptions=eligibleWorkshopDrinks().map(item=>`<option value="${item.id}">${item.name}（${money(item.price)}）</option>`).join(''); return `<section class="calendar-manager"><div><p class="eyebrow">BOOKING MANAGER</p><h3>${eventLabel(event)}｜已預約 ${bookings.length} 位</h3></div><div class="calendar-add-booking"><strong>新增客人預約</strong><div><select id="calendarMembershipPicker" ${full?'disabled':''}><option value="">選擇客人／工作坊票券</option>${available.map(member=>`<option value="${member.id}">${member.customer}｜${member.name}｜剩 ${member.remaining} 次</option>`).join('')}</select><button class="primary" data-calendar-book="${event.id}" ${full?'disabled':''}>加入預約</button></div><small>${full?'這堂課已額滿':available.length?'選擇客人的工作坊票券後加入。':'目前沒有可用的工作坊票券。'}</small></div>${bookings.length?`<div class="booking-manager-list">${bookings.map(booking=>`<div class="booking-manager-row"><div><strong>${booking.customer}</strong><small>${booking.attendedAt?`已出席・${new Date(booking.attendedAt).toLocaleString('zh-TW',{hour12:false})}・已兌換 ${booking.workshopDrink?.name||'免費飲品'}`:'目前預約這堂課'}</small></div><div class="booking-manager-actions">${booking.attendedAt?'':`<select data-workshop-drink="${booking.id}"><option value="">選擇免費 NT$150 飲品</option>${drinkOptions}</select><button class="primary" data-workshop-attend="${booking.id}">確認出席＋兌換</button>`}<button class="secondary" data-member-bookings="${booking.membershipId}">查看所有預約</button><button class="danger" data-booking-action="cancel" data-booking-id="${booking.id}">取消預約</button></div></div>`).join('')}</div>`:'<p class="empty-cart">這堂課目前尚無預約</p>'}</section>`; }
function markWorkshopAttendance(bookingId) { const drinkId=document.querySelector(`[data-workshop-drink="${bookingId}"]`)?.value, drink=eligibleWorkshopDrinks().find(item=>item.id===drinkId), bookings=getWorkshopBookings(), booking=bookings.find(item=>item.id===bookingId); if(!booking||booking.attendedAt)return; if(!drink){alert('請先選擇客人兌換的 NT$150 飲品。');return;} booking.attendedAt=new Date().toISOString(); booking.workshopDrink={id:drink.id,name:drink.name,price:drink.price}; saveWorkshopBookings(bookings); showCalendarEventManager(managedCalendarEventId); renderMenu(); }
...
function addExistingMembership() {
  ...
  const config={ 'vip-5':{type:'VIP',name:'VIP 5 格卡',total:5,tea:'瓜茶'}, 'vip-15':{type:'VIP',name:'VIP 15 格卡',total:15,tea:'瓜茶'}, 'workshop-1':{type:'工作坊',name:'工作坊單堂票',total:1,tea:''}, 'workshop-5':{type:'工作坊',name:'好好生活工作坊五格套票',total:5,tea:''}, 'stored-cr7':{type:'寄杯',name:'CR7 勁能飲寄杯（NT$150）',tea:''}, 'stored-180':{type:'寄杯',name:'特調飲品寄杯（NT$180）',tea:''}, 'stored-250':{type:'寄杯',name:'特調飲品寄杯（NT$250）',tea:''} }[kind];
  ...
  saveMemberships([...getMemberships(),{id:`membership-${Date.now()}-${Math.random().toString(16).slice(2)}`,customer,coach:'',name:config.name,type:config.type,total,remaining,tea:config.tea,bookingCode:config.type==='工作坊'?makeBookingCode():'',usedGrids,purchasedAt,expiresAt}]);
  $('#membershipCustomer').value=''; $('#membershipRemaining').value=config.total||1; showMemberships();
}
let editingOrderId = '';
const escapeField = value => String(value ?? '').replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;');
function orderTimeForEdit(order) {
  if (order.createdAt) { const value=new Date(order.createdAt); if(!Number.isNaN(value.getTime())) return value.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'}); }
  const match=String(order.time||'').match(/(\d{1,2}):(\d{2})/); return match?`${String(match[1]).padStart(2,'0')}:${match[2]}`:'00:00';
}
function showOrderEditor(id) {
  const order=getOrders().find(item=>item.id===id); if(!order)return; editingOrderId=id;
  const isVip=!!order.vip, items=order.items||[];
  const itemFields=items.map((item,index)=>item.category==='vip'?'':`<div class="order-edit-item" data-edit-item="${index}"><input data-edit-name="${index}" value="${escapeField(item.name)}" aria-label="品項名稱" /><input data-edit-qty="${index}" type="number" min="1" value="${item.qty}" aria-label="數量" /><input data-edit-price="${index}" type="number" min="0" value="${item.price}" aria-label="單價" /></div>`).join('');
  const vipFields=isVip?`<section class="order-edit-vip"><strong>VIP 兌換內容</strong><div><label>卡別<select id="editVipCardType"><option value="5" ${String(order.vip.cardType)==='5'?'selected':''}>5 格卡</option><option value="15" ${String(order.vip.cardType)==='15'?'selected':''}>15 格卡</option></select></label><label>第幾格<input id="editVipGrid" type="number" min="1" value="${order.vip.grid||1}" /></label></div><label>搭配茶<input id="editVipTea" value="${escapeField(order.vip.tea)}" /></label><label>口味（用頓號或逗號分開）<input id="editVipFlavor" value="${escapeField((order.vip.flavor||[]).join('、'))}" /></label><label>加強選項（用頓號或逗號分開）<input id="editVipBoost" value="${escapeField((order.vip.boost||[]).join('、'))}" /></label></section>`:'';
  $('#orderEditContent').innerHTML=`<div class="order-edit-meta"><label>日期<input id="editOrderDate" type="date" value="${escapeField(order.date)}" /></label><label>時間<input id="editOrderTime" type="time" value="${orderTimeForEdit(order)}" /></label></div><label class="field-label">客人<input id="editOrderCustomer" value="${escapeField(order.customer)}" /></label><label class="field-label">教練<select id="editOrderCoach"><option value="">未指定</option>${['寶莛','晏玲','婉馨','寶筠','秀瓊','潔蘭','宥靜','素慈／光輝'].map(name=>`<option ${order.coach===name?'selected':''}>${name}</option>`).join('')}</select></label>${vipFields}${isVip?'':`<section class="order-edit-products"><strong>品項、數量、單價</strong><small>可直接修正打錯的品項或金額。</small>${itemFields}</section>`}<label class="field-label">備註<textarea id="editOrderNote" rows="2">${escapeField(order.note)}</textarea></label>`;
  $('#orderEditDialog').showModal();
}
function splitChoices(value) { return String(value||'').split(/[、,，]/).map(item=>item.trim()).filter(Boolean); }
function saveOrderEdit() {
  const orders=getOrders(), order=orders.find(item=>item.id===editingOrderId); if(!order)return;
  const date=$('#editOrderDate').value, time=$('#editOrderTime').value;
  if(!date||!time){alert('請完整填寫日期與時間。');return;}
  order.date=date; order.time=new Date(`${date}T${time}:00`).toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',second:'2-digit'}); order.createdAt=new Date(`${date}T${time}:00`).toISOString(); order.customer=$('#editOrderCustomer').value.trim(); order.coach=$('#editOrderCoach').value; order.note=$('#editOrderNote').value.trim();
  if(order.vip){order.vip.cardType=$('#editVipCardType').value;order.vip.grid=Math.max(1,Number($('#editVipGrid').value)||1);order.vip.tea=$('#editVipTea').value.trim();order.vip.flavor=splitChoices($('#editVipFlavor').value);order.vip.boost=splitChoices($('#editVipBoost').value);order.vip.coach=order.coach;}
  else { let valid=true; (order.items||[]).forEach((item,index)=>{const name=document.querySelector(`[data-edit-name="${index}"]`),qty=document.querySelector(`[data-edit-qty="${index}"]`),price=document.querySelector(`[data-edit-price="${index}"]`);if(!name||!qty||!price)return;const quantity=Math.max(0,Number(qty.value)),unitPrice=Math.max(0,Number(price.value));if(!name.value.trim()||quantity<1){valid=false;return;}item.name=name.value.trim();item.qty=quantity;item.price=unitPrice;}); if(!valid){alert('每個品項都需要名稱與至少 1 份數量。');return;} const freeOrder=!!order.drinkCreditMembershipId||order.items.some(item=>item.category==='workshop-booking');order.total=freeOrder?0:order.items.reduce((sum,item)=>sum+Number(item.price||0)*Number(item.qty||0),0); }
  saveOrders(orders); $('#orderEditDialog').close(); showReport();
}
function deleteOrderRecord(id, context='today') {
  const orders=getOrders(), order=orders.find(item=>item.id===id); if(!order)return;
  const description=order.vip?`VIP ${order.vip.cardType} 格卡第 ${order.vip.grid} 格`:(order.items||[]).map(item=>item.name).join('、');
  if(!confirm(`確定刪除「${order.customer||'未填姓名'}｜${description}」這筆紀錄嗎？\n刪除後無法復原。`))return;
  if(order.vip?.membershipId){
    const memberships=getMemberships(), card=memberships.find(member=>member.id===order.vip.membershipId);
    if(card&&card.usedGrids?.includes(order.vip.grid)){
      card.usedGrids=card.usedGrids.filter(grid=>grid!==order.vip.grid);
      card.remaining=Math.min(card.total,Number(card.remaining||0)+1);
      saveMemberships(memberships);
    }
  }
  saveOrders(orders.filter(item=>item.id!==id));
  if(context==='monthly')showMonthlyReport(); else showReport();
}
function showReport() {
  const orders=todayOrders(), vipUses=orders.filter(o=>o.vip), sales=orders.reduce((s,o)=>s+o.total,0), paidCups=orders.reduce((s,o)=>s+o.items.filter(i=>i.category==='drink').reduce((a,i)=>a+i.qty,0),0), cards=orders.reduce((s,o)=>s+o.items.filter(i=>i.category==='card').reduce((a,i)=>a+i.qty,0),0), workshopCards=orders.reduce((s,o)=>s+o.items.filter(i=>i.category==='workshop-card').reduce((a,i)=>a+i.qty,0),0), bookings=orders.reduce((s,o)=>s+o.items.filter(i=>i.category==='workshop-booking').reduce((a,i)=>a+i.qty,0),0), attendances=getWorkshopBookings().filter(b=>b.attendedAt&&new Date(b.attendedAt).toLocaleDateString('en-CA',{timeZone:'Asia/Taipei'})===today());
  ...
  const activity=[...orders.flatMap(order=>order.vip ? [{time:order.createdAt||`${order.date}T${order.time||'00:00:00'}`,label:`${order.customer||'未填姓名'}｜VIP 兌換套餐 × 1`,detail:`${order.vip.cardType} 格卡第 ${order.vip.grid} 格${order.vip.tea?`・${order.vip.tea}`:''}`}] : order.items.map(item=>({time:order.createdAt||`${order.date}T${order.time||'00:00:00'}`,label:`${order.customer||'未填姓名'}｜${item.name} × ${item.qty}`,detail:order.total?`銷售 ${money(item.price*item.qty)}`:'卡片／寄杯使用'}))),...attendances.map(booking=>({time:booking.attendedAt,label:`${booking.customer}｜工作坊出席`,detail:`免費兌換 ${booking.workshopDrink?.name||'NT$150 飲品'}`}))].sort((a,b)=>String(b.time).localeCompare(String(a.time)));
  $('#reportContent').innerHTML=`<div class="report-grid"><div class="metric"><span>今日營收</span><strong>${money(sales)}</strong></div><div class="metric"><span>單點飲品</span><strong>${paidCups} 杯</strong></div><div class="metric"><span>VIP 卡銷售</span><strong>${cards} 張</strong></div><div class="metric"><span>工作坊套票銷售</span><strong>${workshopCards} 張</strong></div><div class="metric"><span>VIP 兌換</span><strong>${vipUses.length} 筆</strong></div><div class="metric"><span>工作坊出席／飲品</span><strong>${attendances.length} 筆</strong></div><div class="metric"><span>活動預約</span><strong>${bookings} 位</strong></div></div><h3>VIP 今日兌換</h3><div class="report-list vip-report-list">${vipList}</div><h3>今日使用／銷售時間紀錄</h3><div class="report-list">${activity.length?activity.map(item=>`<div class="report-row"><span>${new Date(item.time).toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit',second:'2-digit'})}｜${item.label}<small>${item.detail}</small></span></div>`).join(''):'<p class="empty-cart">今天尚無使用或銷售紀錄</p>'}</div><h3>品項統計</h3><div class="report-list">${Object.keys(count).length ? Object.entries(count).sort((a,b)=>b[1]-a[1]).map(([n,q])=>`<div class="report-row"><span>${n}</span><strong>${q} 份</strong></div>`).join('') : '<p class="empty-cart">今天還沒有結帳紀錄</p>'}</div>`;
  $('#reportContent').insertAdjacentHTML('beforeend',`<h3>修改今日訂單</h3><div class="report-list order-edit-list">${orders.length?orders.slice().sort((a,b)=>String(b.createdAt||b.time).localeCompare(String(a.createdAt||a.time))).map(order=>`<div class="report-row"><span>${order.time||''}｜${order.customer||'未填姓名'}｜${order.vip?`VIP ${order.vip.cardType} 格卡第 ${order.vip.grid} 格`:(order.items||[]).map(item=>item.name).join('、')}</span><div class="order-edit-actions"><button class="secondary" data-edit-order="${order.id}">修改</button><button class="danger" data-delete-order="${order.id}">刪除</button></div></div>`).join(''):'<p class="empty-cart">今天尚無可修改訂單</p>'}</div>`);
...
document.addEventListener('click', e=>{ const membershipDelete=e.target.closest('[data-membership-delete]'); if(membershipDelete){deleteMembership(membershipDelete.dataset.membershipDelete);return;} const attendance=e.target.closest('[data-workshop-attend]'); if(attendance){markWorkshopAttendance(attendance.dataset.workshopAttend);return;} const bookingAction=e.target.closest('[data-booking-action]'); if(bookingAction){if(bookingAction.dataset.bookingAction==='save')updateWorkshopBooking(bookingAction.dataset.bookingId);if(bookingAction.dataset.bookingAction==='cancel')cancelWorkshopBooking(bookingAction.dataset.bookingId);return;} const workshop=e.target.closest('[data-workshop-event]'); if(workshop) selectWorkshopEvent(workshop.dataset.workshopEvent); const product=e.target.closest('.product'); if(product) addProduct(product.dataset.id); const action=e.target.dataset.action; if(action){const line=cart.find(i=>i.id===e.target.dataset.id); if(action==='plus')line.qty++; if(action==='minus')line.qty===1?cart=cart.filter(i=>i!==line):line.qty--; if(action==='remove')cart=cart.filter(i=>i!==line); renderCart();} const grid=e.target.closest('[data-grid]'); if(grid){vipGrid=Number(grid.dataset.grid);renderVipGrid();renderCart();} const choice=e.target.closest('[data-vip-choice] button'); if(choice){const type=choice.closest('[data-vip-choice]').dataset.vipChoice; choice.classList.toggle('active'); vipSelections[type]=[...choice.parentElement.querySelectorAll('.active')].map(b=>b.textContent); } if(e.target.matches('[data-close]'))e.target.closest('dialog').close(); });
document.addEventListener('change',e=>{const picker=e.target.closest('[data-cart-drink-mode]');if(picker)setCartDrinkMode(picker.dataset.cartDrinkMode,picker.value);});
document.addEventListener('click',e=>{const edit=e.target.closest('[data-edit-order]');if(edit){showOrderEditor(edit.dataset.editOrder);return;}const remove=e.target.closest('[data-delete-order]');if(remove)deleteOrderRecord(remove.dataset.deleteOrder,remove.dataset.deleteContext);});
document.querySelectorAll('.tab').forEach(t=>t.addEventListener('click',()=>{const wasVip=vipActive, wasWorkshop=['workshop','calendar'].includes(activeCategory);activeCategory=t.dataset.category;vipActive=activeCategory==='vip';if(vipActive)cart=[{id:'vip-bundle',name:'VIP 兌換套餐',category:'vip',price:0,qty:1}];else if(['workshop','calendar'].includes(activeCategory)){cart=[];selectedWorkshopEvent=null;$('#workshopSelected').textContent='請從左側選擇欲預約的課程';syncWorkshopMemberPicker();}else if(wasVip||wasWorkshop)cart=[];document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===t));renderVipControls();if(vipActive){syncVipMemberships();renderVipGrid();}renderMenu();renderCart();}));
$('#newOrder')?.addEventListener('click',resetOrder); $('#showReports')?.addEventListener('click',showReport); $('#showMonthlyReports')?.addEventListener('click',showMonthlyReport); $('#monthlyReportMonth')?.addEventListener('change',showMonthlyReport); $('#showMemberships')?.addEventListener('click',showMemberships); $('#addMembership')?.addEventListener('click',addExistingMembership); $('#membershipSearch')?.addEventListener('input',showMemberships); $('#membershipSort')?.addEventListener('change',showMemberships); $('#membershipKind')?.addEventListener('change',()=>{const kind=$('#membershipKind').value;$('#membershipRemaining').value=kind==='vip-15'?15:kind.startsWith('stored-')?1:kind==='workshop-1'?1:5;}); $('#exportData')?.addEventListener('click',exportCSV);
...
$('#vipMembership').addEventListener('change',()=>{const selected=selectedVipMembership();if(selected){$('#vipCardType').value=String(selected.total);vipGrid=selected.usedGrids.length+1;$('#vipCoachInfo').textContent=`所屬教練：${selected.coach||'未指定'}`;$('#vipCoachInfo').classList.remove('hidden');}else $('#vipCoachInfo').classList.add('hidden');renderVipGrid();renderCart();});
