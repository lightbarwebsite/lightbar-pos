...
function membershipExpiryDate(member) { const date=new Date(`${member.purchasedAt||today()}T12:00:00`); date.setMonth(date.getMonth()+(member.type==='工作坊'?2:1)); return date.toLocaleDateString('en-CA'); }
...
async function syncToCloud() { if(loadingCloud)return;try{setSyncStatus('正在同步…','busy');const response=await fetch(SYNC_ENDPOINT,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({action:'sync',orders:getOrders(),memberships:getMemberships(),bookings:getWorkshopBookings()})});const result=await response.json();if(!result.ok)throw new Error(result.error||'同步失敗');setSyncStatus('已同步','ok');}catch(error){console.warn('Cloud sync failed:',error);setSyncStatus('離線：資料已留在本機','error');} }
...
async function loadCloudData() { if(loadingCloud)return;loadingCloud=true;try{setSyncStatus('正在載入雲端…','busy');const response=await fetch(`${SYNC_ENDPOINT}?action=bootstrap`);const result=await response.json();if(!result.ok)throw new Error(result.error||'載入失敗');const orders=mergeRecords(getOrders(),result.orders||[]),memberships=mergeRecords(getMemberships(),result.memberships||[]),bookings=mergeRecords(getWorkshopBookings(),result.bookings||[]);localStorage.setItem('store-pos-orders',JSON.stringify(orders));localStorage.setItem('store-pos-memberships',JSON.stringify(memberships));localStorage.setItem('store-pos-workshop-bookings',JSON.stringify(bookings));renderMenu();renderVipControls();renderCart();setSyncStatus('已連線','ok');queueCloudSync();}catch(error){console.warn('Cloud load failed:',error);setSyncStatus('離線：使用本機資料','error');}finally{loadingCloud=false;} }
...
function renderMenu() {
  ...
  $('#menuGrid').innerHTML = products.filter(p => activeCategory === 'workshop' ? ['workshop','workshop-card'].includes(p.category) : p.category === activeCategory).map(p => `<button class="product" data-id="${p.id}">${p.photo ? `<span class="product-photo"><img src="assets/menu/${p.photo}" alt="${p.name} 飲品照片" style="object-position:${p.focus}" /></span>` : `<span class="emoji">${p.emoji}</span>`}<span class="product-info"><strong>${p.name}</strong><small>${p.detail}</small><b>${money(p.price)}</b></span><span class="add">+</span></button>`).join('');
}
...
function addProduct(id) { const p = products.find(x=>x.id===id); const line = cart.find(x=>x.id===id); line ? line.qty++ : cart.push({...p,qty:1}); if(p.category==='workshop') syncWorkshopMemberships(); renderCart(); }
...
function createMemberships(order) {
  ...
  order.items.filter(i => i.category === 'card' || i.category === 'workshop-card').forEach(item => {
    ...
    for (let n=0; n<item.qty; n++) { const purchasedAt=order.date, expiresAt=membershipExpiryDate({type,purchasedAt}); memberships.push({ id:`membership-${Date.now()}-${Math.random().toString(16).slice(2)}`, customer:order.customer, name:item.name, type, total, remaining:total, tea:item.tea||'', bookingCode:type==='工作坊'?makeBookingCode():'', usedGrids:[], purchasedAt, expiresAt }); }
  });
...
function addCalendarBooking(eventId) { const membershipId=$('#calendarMembershipPicker')?.value, event=getWorkshopEvents().find(item=>item.id===eventId), memberships=getMemberships(), member=memberships.find(item=>item.id===membershipId), bookings=getWorkshopBookings(); if(!membershipId||!event||!member){alert('請先選擇客人的工作坊票券。');return;} if(member.remaining<1||isMembershipExpired(member)){alert('這張票券目前無法使用。');return;} if(bookings.filter(item=>item.eventId===eventId).length>=event.capacity){alert('這堂課已額滿。');return;} bookings.push({id:`booking-${Date.now()}-${Math.random().toString(16).slice(2)}`,eventId,membershipId,customer:member.customer,createdAt:new Date().toISOString(),source:'staff-calendar'}); member.remaining-=1; saveMemberships(memberships); saveWorkshopBookings(bookings); renderMenu(); renderCart(); showCalendarEventManager(eventId); }
...
function membershipGroupsMarkup(memberships, emptyMessage) {
  ...
  return `<div class="membership-list">${Object.entries(groups).map(([customer,cards])=>`<section class="membership-group"><div class="membership-group-header"><strong>${customer}</strong><span>${cards.length} 張卡／票券</span></div>${cards.map(member=>{const unit=member.type==='VIP'?'格':'次',expired=isMembershipExpired(member);return `<article class="membership-row"><div class="membership-card-title"><span class="membership-kind">${member.type}</span>${expired?'<span class="membership-expired">已到期</span>':''}<strong>${member.name}</strong><small>${membershipLabel(member)}・有效至 ${member.expiresAt||membershipExpiryDate(member)}${member.type==='工作坊'?`・預約碼 ${member.bookingCode||'產生中…'}`:''}</small></div><div class="membership-balance"><label>剩餘<input type="number" min="0" max="${member.total}" value="${member.remaining}" data-membership-remaining="${member.id}" aria-label="${customer} ${member.name} 剩餘${unit}" /></label><span>／${member.total} ${unit}</span><button class="secondary membership-save" data-membership-save="${member.id}">儲存</button></div></article>`;}).join('')}</section>`).join('')}</div>`;
}
function showMemberships() {
  const query=($('#membershipSearch')?.value||'').trim().toLocaleLowerCase(), all=getMemberships().sort((a,b)=>a.customer.localeCompare(b.customer,'zh-Hant')||String(a.purchasedAt).localeCompare(String(b.purchasedAt)));
...
function updateMembershipRemaining(id) { const input=document.querySelector(`[data-membership-remaining="${id}"]`), membership=getMemberships().find(item=>item.id===id); if(!input||!membership)return; membership.remaining=Math.max(0,Math.min(membership.total,Number(input.value)||0)); if(membership.type==='VIP') membership.usedGrids=Array.from({length:membership.total-membership.remaining},(_,index)=>index+1); saveMemberships(getMemberships()); showMemberships(); }
function addExistingMembership() {
  ...
  const config={ 'vip-5':{type:'VIP',name:'VIP 5 格卡',total:5,tea:'瓜茶'}, 'vip-15':{type:'VIP',name:'VIP 15 格卡',total:15,tea:'瓜茶'}, 'workshop-1':{type:'工作坊',name:'工作坊單堂票',total:1,tea:''}, 'workshop-5':{type:'工作坊',name:'工作坊五次套票',total:5,tea:''} }[kind];
  const remaining=Math.min(config.total,Math.max(0,Number($('#membershipRemaining').value)||0));
  ...
  saveMemberships([...getMemberships(),{id:`membership-${Date.now()}-${Math.random().toString(16).slice(2)}`,customer,name:config.name,type:config.type,total:config.total,remaining,tea:config.tea,bookingCode:config.type==='工作坊'?makeBookingCode():'',usedGrids,purchasedAt,expiresAt}]);
  $('#membershipCustomer').value=''; $('#membershipRemaining').value=config.total; showMemberships();
}
function showReport() {
