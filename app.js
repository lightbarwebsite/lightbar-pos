...
function renderMenu() {
  ...
  $('#menuGrid').innerHTML = products.filter(p => activeCategory === 'workshop' ? ['workshop','workshop-card'].includes(p.category) : p.category === activeCategory).map(p => { const left=p.charityType?charitySlotsLeft(p.charityType):null, soldOut=left===0, detail=p.charityType?`${p.detail}・剩 ${left} 位`:p.detail; return `<button class="product" data-id="${p.id}" ${soldOut?'disabled':''}>${p.photo ? `<span class="product-photo"><img src="assets/menu/${p.photo}" alt="${p.name} 飲品照片" style="object-position:${p.focus}" /></span>` : `<span class="emoji">${p.emoji}</span>`}<span class="product-info"><strong>${p.name}</strong><small>${detail}</small><b>${soldOut?'已額滿':money(p.price)}</b></span><span class="add">+</span></button>`; }).join('');
}
