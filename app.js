...
function showReport() {
  ...
  const activity=[...orders.flatMap(order=>order.items.map(item=>({time:order.createdAt||`${order.date}T${order.time||'00:00:00'}`,label:`${order.customer||'未填姓名'}｜${item.name} × ${item.qty}`,detail:order.vip?'VIP 餐卡兌換':order.total?`銷售 ${money(item.price*item.qty)}`:'卡片／寄杯使用'}))),...attendances.map(booking=>({time:booking.attendedAt,label:`${booking.customer}｜工作坊出席`,detail:`免費兌換 ${booking.workshopDrink?.name||'NT$150 飲品'}`}))].sort((a,b)=>String(b.time).localeCompare(String(a.time)));
  ...
  $('#reportContent').insertAdjacentHTML('beforeend',`<h3>修改今日訂單</h3><div class="report-list order-edit-list">${orders.length?orders.slice().sort((a,b)=>String(b.createdAt||b.time).localeCompare(String(a.createdAt||a.time))).map(order=>`<div class="report-row"><span>${order.time||''}｜${order.customer||'未填姓名'}｜${order.vip?`VIP ${order.vip.cardType} 格卡第 ${order.vip.grid} 格`:(order.items||[]).map(item=>item.name).join('、')}</span><button class="secondary" data-edit-order="${order.id}">修改</button></div>`).join(''):'<p class="empty-cart">今天尚無可修改訂單</p>'}</div>`);
...
document.addEventListener('click',e=>{const edit=e.target.closest('[data-edit-order]');if(edit)showOrderEditor(edit.dataset.editOrder);});
